...
write.csv(NewSalaryWithVariance.tidy,"/Users/tracy/Desktop/STAT5702_NYC_Employment_Analysis/DataPreprocessing/Data/Preprocess_ClevelandDotPlot/NewSalaryWithVariance.csv", row.names = FALSE)
# draw cleveland dot plot according to variance

NewSalaryWithVariance <- read_csv("/Users/tracy/Desktop/STAT5702_NYC_Employment_Analysis/DataPreprocessing/Data/Preprocess_ClevelandDotPlot/NewSalaryWithVariance.csv")
...
CountySalary <- CountySalary[CountySalary$index == "Max Salary" | CountySalary$index == "Min Salary",]
CountySalary <- CountySalary %>% select(1,2,6) 
...
```{r}
CountyYearlySalary <- NewSalary %>% select(1, 3, 9)
```


## Variations in salaries between different counties
use the biggest 3 counties to minus the smallest one, divide by the salary of the smallest one and then add them up.
