...
write.csv(GenderData.tidy,"/Users/tracy/Desktop/STAT5702_NYC_Employment_Analysis/DataPreprocessing/salary/TidyGenderData.csv", row.names = FALSE)
...
TotalData <- merge(YearlyDataWithVariance,GenderData.tidy[c(1,7)],by="Occupations")

ggplot(TotalData,aes(x=fct_reorder(Occupations, variance_pct,.desc = T), y = variance_pct)) +
  geom_col(fill = ifelse(TotalData$MaleDominate == 1,"orange","green"))+
    theme(axis.text=element_text(size=30,angle = 90, vjust = 0.5, hjust=1),
      axis.title=element_text(size=40,face="bold"),
      plot.title = element_text(size = 50, face = "bold"))+
