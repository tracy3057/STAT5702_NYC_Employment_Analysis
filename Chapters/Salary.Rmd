...
NewSalaryWithVariance <- NewSalaryWithVariance[NewSalaryWithVariance[, "year"] == "year2010",]

NewSalaryWithVariance <- NewSalaryWithVariance %>% 
  mutate(variance_pct = (variance/Salary_YearlyAvg))

NewSalaryWithVariance1 <- NewSalaryWithVariance %>%select(1,5) %>% unique()
...
ggplot(NewSalaryWithVariance1,aes(x=fct_reorder(Occupations, abs(variance_pct)), y = variance_pct)) +
  ...
  coord_flip()+
    theme(axis.text=element_text(),
      ...
      plot.title = element_text(face = "bold"))+ xlab('Occupations') + ylab('Difference_pct') + ggtitle('Difference in Percentage of Salaries over Years')+
  mytheme
```
...
ggplot(CountySalary3,aes(x=fct_reorder(Occupations, variance_pct), y = variance_pct)) +
  geom_col(fill ="royalblue3",alpha = 0.75)+
  ylab('Difference_pct') + xlab('Occupations') +
  coord_flip()+ 
  ggtitle('Difference in Percentage of Salaries in Different Counties') +
  mytheme
```
### Top 5 in Variation
...
ggplot(GenderSalary,aes(x=fct_reorder(Occupations, abs(variance_pct)), y = variance_pct)) +
  ...
  coord_flip()+
    theme(axis.text=element_text(),
      axis.title=element_text(face="bold"),
      plot.title = element_text(face = "bold"))+ xlab('Difference_pct') + ylab('Occupations') + ggtitle('Difference in Percentage of Salaries for Different Genders')+
  mytheme
